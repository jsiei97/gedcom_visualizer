...
def format_individual(individual):
    ...
    info = f"ID: {pointer}\n"
    info += (
        f"  Name: {name[0] if name else 'Unknown'} {name[1] if len(name) > 1 else ''}\n"
    )
...
def list_all_individuals(gedcom_parser):
    ...
    individual_elements = [
        elem for elem in individuals if isinstance(elem, IndividualElement)
    ]
...
def main():
    ...
    parser.add_argument(
        "-s", "--search", help="Search for individuals by name", metavar="TERM"
    )
    ...
    try:
        ...
        print(
            "\nThis might be due to GEDCOM format violations in the file.",
            file=sys.stderr,
        )
